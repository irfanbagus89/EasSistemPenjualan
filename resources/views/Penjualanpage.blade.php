                    ...
                    <div class="w-full h-auto flex gap-12 flex-wrap py-4">
                       {{-- Start Card --}}
                        ...
                        <div class="card card-compact w-96 bg-base-100 shadow-xl">
                            <figure><img src="{{asset('storage/'.$dt->foto)}}" alt="{{$dt->foto}}" /></figure>
                            <div class="card-body">
                            <h2 class="card-title">{{$dt->nama}}!</h2>
                            <h3>{{$dt->deskripsi}}</h3>
                            <h4>Rp {{$dt->harga}}</h4>
                            <form action="/addCart/{{$dt->id_obat}}" method="get">
                ...
                <div class="w-[22%] h-[35rem] rounded-2xl shadow-xl p-4 relative">
                    <div class="w-full min-h-[23rem] relativ">
                        <div class="absolute right-5 top-0" method="get">
                            @if(isset($obatList) && $obatList->isNotEmpty())
                                @foreach ($obatList as $cart) 
                                <a href="/cart/hapus/{{$cart->id_detail}}">
                                    <img src="{{asset('assets/x.svg')}}" class="w-4 h-4 block mb-28" alt="">
                        ...
                        @if(isset($obatList) && $obatList->isNotEmpty())
                            ...
                            <div class="w-full h-24 rounded-xl shadow-xl flex gap-4 items-center px-4 mb-4">
                                <img src="{{asset('storage/'.$cart->obat->foto)}}" class="w-16 h-16 rounded-2xl" alt="">
                                <div class="w-40 h-20">
                                    <h2>{{$cart->obat->nama}}</h2>
                                    <h3>{{$cart->obat->harga}}</h3>
                                </div>
                                <div class="">
                                    <input type="text" class="border w-10 text-center" id="qty_{{ $cart->id_obat }}" name="qty_{{ $cart->id_obat }}" min="1" readonly value="{{$cart->qty}}">
                                </div>
                            </div>
                            @endforeach
                        @else
                        ...
                        @endif
                            ...
                            <div class="w-full">
                                @if(isset($totalHarga) && $totalHarga > 0)
                                <label class="label">
                                <span class="label-text">Total</span>
                                </label>
                                <input type="text" class="input input-bordered w-full max-w-xs h-8" value="{{$totalHarga}}" readonly/>
